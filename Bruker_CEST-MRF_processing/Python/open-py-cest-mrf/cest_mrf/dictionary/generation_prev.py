...
def inverse_key_map(key):
    ...
    """
    inverse_mapping = {v: k for k, v in {'tw1': 't1w', 'tw2': 't2w', 'fww': 'f',
                                         'ts1': 't1s', 'ts2': 't2s', 'fss': 'fs', 'ksw': 'ksw',
                                         'tm1': 't1m', 'tm2': 't2m', 'fmm': 'fm', 'lmm': 'lineshape'}.items()}
...
def prepare_dictionary(dict_, equals=None):
    ...
    if equals is not None:
        ...
        for pair_factor in equals:
            ...
            if pair_factor[0] not in var_names or pair_factor[1] not in var_names:
                raise ValueError(f"Key {key_map(pair_factor[0])} or {key_map(pair_factor[1])} not in dictionary variables: {var_names}")
